...
# Q. 다음 전화번호부에서 서울에 사는 사람은 몇 명인지 구하세요. (02)
...
# print(f'서울 번호 {seoul}개')



# Q. 로제 - on the ground 가사 중 한 번만 등장한 단어를 출력하세요.

# lyrics = """my life is  been magic  seems fantastic
...
# 방탈출 문제 1
# Q. 2**1004에서 str(1의 등장 횟수) + str(3의 등장 횟수)
...
# print(str(one) + str(three))



# 방탈출 문제 2
# Q. 1에서 12345까지 숫자를 센다고 하자.
# 1, 2, 3, 4, 5 각각의 등장 횟수의 합
...
# Q. 영문장을 입력받고, 첫 글자를 대문자로 바꿔주는 프로그램

# sen = input('영문장 입력: ')

# sen = sen.replace(sen[0], sen[0].upper())
...
# 문자열.isalpha( )
# > 문자열이 알파벳으로 이루어짐 True, 아니면 False

# 문자열.isnumeric( )
# > 문자열이 숫자로 이루어짐 True, 아니면 False

# Q. 문자열을 입력받고, 문자열 안에 알파벳의 개수와 숫자의 개수를 출력해라~

# ex1) 입력 : 1q2w3e4r !@
#       알파벳 4개 숫자 4개   (!@는알파벳도숫자도아니니까제외)
# ex2) 입력 : hello 2022~ goodbye 2021!
#       알파벳 12개 숫자 8개
...
# Q. 문자열을 입력받고, 위처럼 유니코드로 변역되도록 프로그램하세요.
...
# 대상마다 각각의 수에 접근해서 홀수, 짝수 판단했을 때
# 짝수가 끼어 있으면 홀수로만 이루어진 수가 아닌 것
# 1) 도중에 짝수가 발견되면 중단
# 2) 홀수의 개수를 세고, 숫자의 길이랑 비교
...
#     if (sum_ - last**2) % 10 == last:
#         koi += 1

# print(koi)


# 강사님 풀이: split(), 슬라이싱 사용
# 제품 코드가 \n으로 구분된 문자열로 주어짐
